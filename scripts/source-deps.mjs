...
else fail(`unknown command ${JSON.stringify(command)}; use prepare, restore, status, or watch`);
...
async function prepare() {
  ...
  if (sourceManifest.scripts?.["source:prepare"]) run("bun", ["run", "source:prepare"], sourceDir);
  if (!sourceManifest.scripts?.build) fail(`${dependency.packageName} source checkout has no build script: ${sourceDir}`);
  ...
  await writeFile(stateFile, `${JSON.stringify({ packageName: dependency.packageName, sourcePackageName: sourceManifest.name, sourceDir, revision }, null, 2)}\n`);
  process.stdout.write(`source dependency ready: ${dependency.packageName} -> ${sourceDir} @ ${revision.slice(0, 12)}\n`);
}
...
async function status() {
  ...
  let state;
  try { state = JSON.parse(await readFile(stateFile, "utf8")); } catch { state = undefined; }
...
async function watch() {
  ...
  const result = spawnSync("bun", ["run", "build", "--", "--watch"], { cwd: sourceDir, env: process.env, stdio: "inherit" });
  process.exit(result.status ?? 1);
}

async function readSourceManifest() {
  try { await access(path.join(sourceDir, "package.json")); }
  catch { fail(`missing ${dependency.packageName} source checkout at ${sourceDir}; set ${dependency.sourceEnv} to override`); }
  const manifest = JSON.parse(await readFile(path.join(sourceDir, "package.json"), "utf8"));
  if (!dependency.acceptedSourceNames.includes(manifest.name)) fail(`expected ${dependency.acceptedSourceNames.join(" or ")} at ${sourceDir}, found ${manifest.name ?? "unnamed package"}`);
  return manifest;
}

async function isSymlink(filePath) {
  try { return (await lstat(filePath)).isSymbolicLink(); }
  catch { return false; }
}

function run(executable, args, cwd, capture = false) {
  const result = spawnSync(executable, args, { cwd, env: process.env, encoding: capture ? "utf8" : undefined, stdio: capture ? ["ignore", "pipe", "inherit"] : "inherit" });
